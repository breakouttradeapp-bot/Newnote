...
class AddEditNoteActivity : AppCompatActivity() {
    ...
    private val speechRecognitionLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            ...
            if (!matches.isNullOrEmpty()) {
                ...
                val cursorPos = binding.etContent.selectionStart.coerceAtLeast(0)
                val newText = currentText.substring(0, cursorPos) + 
                             matches[0] + " " + 
                             currentText.substring(cursorPos)
                binding.etContent.setText(newText)
                binding.etContent.setSelection((cursorPos + matches[0].length + 1).coerceAtMost(newText.length))
            }
        }
    }
    ...
    private fun setupRichTextToolbar() {
        ...
        binding.btnVoiceInput.setOnClickListener { startVoiceRecognition() }
        binding.btnTextToSpeech.setOnClickListener { speakNoteContent() }
        ...
        binding.etContent.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                val text = s?.toString() ?: ""
                val wc = RichTextUtils.countWords(text)
                val charCount = text.length
                binding.tvWordCount.text = "$wc words · $charCount chars · ${RichTextUtils.estimateReadTime(wc)}"
            }
        })
    }
    ...
    private fun speakNoteContent() {
        ...
        val textToSpeak = if (title.isNotEmpty()) {
            "$title. $content"
        } else {
            content
        }
    ...
    private fun showFontPicker() {
        ...
        val fontResources = arrayOf(
            null, // null means default system font
        ...
        AlertDialog.Builder(this)
            ...
            .setItems(fontNames) { _, which ->
                val fontRes = fontResources[which]
                if (fontRes == null) {
                    // Reset to default system font
                    binding.etTitle.typeface = android.graphics.Typeface.DEFAULT
                    binding.etContent.typeface = android.graphics.Typeface.DEFAULT
                } else {
                    try {
                        val typeface = androidx.core.content.res.ResourcesCompat.getFont(this, fontRes)
                        binding.etTitle.typeface = typeface
                        binding.etContent.typeface = typeface
                        showToast("Font changed to ${fontNames[which]}")
                    } catch (e: Exception) {
