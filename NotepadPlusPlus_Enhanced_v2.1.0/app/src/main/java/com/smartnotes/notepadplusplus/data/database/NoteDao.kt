...
interface NoteDao {
    ...
    // FIXED COLLATE POSITION
    @Query("SELECT * FROM notes WHERE is_archived = 0 ORDER BY is_pinned DESC, title COLLATE NOCASE ASC")
    ...
    @Query("SELECT DISTINCT label FROM notes WHERE label != '' AND is_archived = 0 ORDER BY label COLLATE NOCASE ASC")
